...
{
    boss_ichoronAI(Creature* c) : BossAI(c, BOSS_ICHORON) { }
    ...
    void JustSummoned(Creature* summoned) override
    {
        if (summoned)
        {
            ...
            me->CastSpell(summoned, SPELL_CREATE_GLOBULE_VISUAL, true);
...
struct npc_ichor_globuleAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        events.RescheduleEvent(EVENT_RANGE_CHECK, 1s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (events.ExecuteEvent() == EVENT_RANGE_CHECK)
        {
            if (Creature* ichoron = Instance->GetCreature(BOSS_ICHORON))
...
{
    RegisterVioletHoldCreatureAI(boss_ichoronAI);
    RegisterVioletHoldCreatureAI(npc_ichor_globuleAI);
}
