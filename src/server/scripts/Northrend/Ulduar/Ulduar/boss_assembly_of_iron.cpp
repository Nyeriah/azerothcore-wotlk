...
{
    boss_steelbreaker(Creature* c) : ScriptedAI(c)
    {
...
{
    boss_runemaster_molgeim(Creature* c) : ScriptedAI(c), summons(me)
    {
        pInstance = c->GetInstanceScript();
    }
...
struct npc_assembly_lightning : public ScriptedAI
{
    npc_assembly_lightning(Creature* c) : ScriptedAI(c)
    {
        _boomed = false;
    }
...
{
    boss_stormcaller_brundir(Creature* c) : ScriptedAI(c)
    {
        pInstance = c->GetInstanceScript();
    }
...
void AddSC_boss_assembly_of_iron()
{
    ...
    RegisterUlduarCreatureAI(npc_assembly_lightning);
